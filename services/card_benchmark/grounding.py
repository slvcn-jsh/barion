...
from .analysis import STOP, tokens
...
def ground_claims(card: NormalizedCard, claims: list[Claim], segments: list[Segment]) -> list[GroundingResult]:
    ...
    for claim in claims:
        ...
        citation_valid = bool(cited_text and referenced and all(
            resolve_source_span(
                by_id[reference.segmentId].text, reference.evidenceText,
                expected_source_text_sha256=reference.sourceTextSha256 or None,
            ).status in {"exact", "normalized", "context-disambiguated"}
            for reference in references if reference.evidenceText and reference.segmentId in by_id
        ))
        ...
        citation_status = _citation_status(card, claim, cited_text, citation_valid, citation, segment)
...
def _citation_status(card: NormalizedCard, claim: Claim, cited_text: str, valid: bool, citation, segment):
...
def _assess_many(claim: Claim, segments: list[Segment], tier: str):
    ...
    for segment in segments:
        ...
        if assessment[0] in {"contradicted", "supported"}: return assessment
...
def _assess(claim: Claim, evidence: str, segment: Segment | None = None, tier: str = "citation"):
    ...
    if best_score >= 0.45 or (_entity_overlap(claim.claimText, evidence) and numbers_match):
...
def _contradiction(claim: Claim, evidence: str) -> str:
    ...
    for left, right in _ANTONYMS:
        ...
        # Comparison/table windows often contain both values. Bare co-occurrence cannot assign value to entity.
